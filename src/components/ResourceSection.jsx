  ...
  return <section className="content-section resource-section" aria-labelledby={`${section.id}-heading`}>
    ...
    <div className="resource-groups">
      {section.groups.map((group) => {
        const hasGroupTitle = Boolean(group.title?.trim() || group.icon);

        return <div className="resource-group" key={group.title || group.links.map((link) => link.label).join("-")}>
          {hasGroupTitle ? <span className="resource-group-title">
