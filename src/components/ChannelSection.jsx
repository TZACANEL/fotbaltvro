...
function ChannelSection({ config, channels = [], buttons, labels, id, headingLevel, sectionClassName = "", gridClassName = "", limit }) {
  ...
  return <section className={`content-section ${sectionClassName}`.trim()} id={id} aria-labelledby={`${id}-heading`}>
    ...
    {listedChannels.length ? <div className="channel-list-shell"><div className={`channel-grid ${gridClassName}`.trim()}>{listedChannels.map((channel) => <ChannelCard key={channel.id} channel={channel} buttonLabel={buttons.sourceLabel} invalidUrlLabel={buttons.invalidUrlLabel} labels={labels} />)}</div></div> : <p className="empty-state">{config.emptyMessage}</p>}
  </section>;
}
